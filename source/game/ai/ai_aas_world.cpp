...
AiAasWorld::~AiAasWorld()
{
    ...
    if (bboxes) G_LevelFree(bboxes);
    if (vertexes) G_LevelFree(vertexes);
    if (planes) G_LevelFree(planes);
    if (edges) G_LevelFree(edges);
    if (edgeindex) G_LevelFree(edgeindex);
    if (faces) G_LevelFree(faces);
    if (faceindex) G_LevelFree(faceindex);
    if (areas) G_LevelFree(areas);
    if (areasettings) G_LevelFree(areasettings);
    if (reachability) G_LevelFree(reachability);
    if (nodes) G_LevelFree(nodes);
    if (portals) G_LevelFree(portals);
    if (portalindex) G_LevelFree(portalindex);
    if (clusters) G_LevelFree(clusters);
}
