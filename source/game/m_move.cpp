...
// the midpoint must be within 16 of the bottom
	...
	G_Trace (&trace, start, vec3_origin, vec3_origin, stop, ent, MASK_MONSTERSOLID);
...
// flying monsters don't step up
	...
	// try one move with vertical motion, then one without
		for (i=0 ; i<2 ; i++)
		{
			...
			G_Trace (&trace, ent->s.origin, ent->r.mins, ent->r.maxs, neworg, ent, MASK_MONSTERSOLID);
...
// push down from a step height above the wished position
	...
	G_Trace (&trace, neworg, ent->r.mins, ent->r.maxs, end, ent, MASK_MONSTERSOLID);
