...
* CanTeleportPlayer
*
* Checks if teleporters can teleport the player.
*/
...
void TeleportPlayer( edict_t *player, edict_t *dest )
{
	...
	VectorCopy( player->r.client->ps.pmove.velocity, velocity );

	velocity[2] = 0; // ignore vertical velocity
	speed = VectorLengthFast( velocity );

	AnglesToAxis( dest->s.angles, axis );
	VectorScale( &axis[AXIS_FORWARD], speed, player->r.client->ps.pmove.velocity );

	VectorCopy( dest->s.angles, player->r.client->ps.viewangles );
	VectorCopy( dest->s.origin, player->r.client->ps.pmove.origin );

	// set the delta angle
	for( i = 0; i < 3; i++ )
		player->r.client->ps.pmove.delta_angles[i] = ANGLE2SHORT( player->r.client->ps.viewangles[i] ) - player->r.client->ucmd.angles[i];

	player->r.client->ps.pmove.pm_flags |= PMF_TIME_TELEPORT;
	player->s.teleported = qtrue;
	player->r.client->ps.pmove.pm_time = 1; // force the minimum no control delay

	// update the entity from the pmove
	VectorCopy( player->r.client->ps.viewangles, player->s.angles );
	VectorCopy( player->r.client->ps.pmove.origin, player->s.origin );
	VectorCopy( player->r.client->ps.pmove.origin, player->s.old_origin );
	VectorCopy( player->r.client->ps.pmove.origin, player->olds.origin );
	VectorCopy( player->r.client->ps.pmove.velocity, player->velocity );
