...
void RegisterMathAddon( asIScriptEngine *engine ) {
	...
	math_asGlobFuncs[] =
	{
		...
		{ "int64 max( int64 x, int64 y )", asFUNCTION( asFunc_imax ) },
		{ "int64 min( int64 x, int64 y )", asFUNCTION( asFunc_imin ) },
		{ "uint64 max( uint64 x, uint64 y )", asFUNCTION( asFunc_umax ) },
		{ "uint64 min( uint64 x, uint64 y )", asFUNCTION( asFunc_umin ) },
