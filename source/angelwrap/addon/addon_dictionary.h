...
#include <string>
typedef std::string realDictKey_t;
typedef asstring_t	dictKey_t;
...
typedef std::unordered_map<realDictKey_t, AS_NAMESPACE_QUALIFIER CScriptDictValue> dictMap_t;
#else
#include <map>
typedef std::map<realDictKey_t, AS_NAMESPACE_QUALIFIER CScriptDictValue> dictMap_t;
#endif
...
public:
	...
	CScriptDictValue *operator[](const realDictKey_t &key);
	const CScriptDictValue *operator[](const realDictKey_t &key) const;

	// Returns the type id of the stored value, or negative if it doesn't exist
	int GetTypeId(const realDictKey_t &key) const;

	// Returns true if the key is set
	bool Exists(const realDictKey_t &key) const;
	...
	// Deletes the key
	bool Delete(const realDictKey_t &key);
