...
CScriptDictValue *CScriptDictionary::operator[](const realDictKey_t &key)
{
	// Return the existing value if it exists, else insert an empty value
	return &dict[key];
}

const CScriptDictValue *CScriptDictionary::operator[](const realDictKey_t &key) const
{
...
int CScriptDictionary::GetTypeId(const realDictKey_t &key) const
{
	dictMap_t::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
bool CScriptDictionary::Delete(const realDictKey_t &key)
{
	dictMap_t::iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
CScriptArray* CScriptDictionary::GetKeys() const
{
	...
	for( it = dict.begin(); it != dict.end(); it++ )
	{
		...
		*( (dictKey_t **)array->At(current) ) = objectString_FactoryBuffer( c_str, it->first.length() );
	}
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("Dictionary", "void set(const String &in, const ?&in)", asMETHODPR(CScriptDictionary,Set,(const asstring_t&,void*,int),void), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("Dictionary", "void set(const String &in, const int64&in)", asMETHODPR(CScriptDictionary,Set,(const asstring_t&,const asINT64&),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("Dictionary", "bool get(const String &in, int64&out) const", asMETHODPR(CScriptDictionary,Get,(const asstring_t&,asINT64&) const,bool), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("Dictionary", "void set(const String &in, const double&in)", asMETHODPR(CScriptDictionary,Set,(const asstring_t&,const double&),void), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("Dictionary", "bool exists(const String &in) const", asMETHODPR(CScriptDictionary,Exists,(const asstring_t&) const, bool), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("Dictionary", "bool delete(const String &in)", asMETHODPR(CScriptDictionary,Delete,(const asstring_t&), bool), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("Dictionary", "DictionaryValue &opIndex(const String &in)", asMETHODPR(CScriptDictionary, operator[], (const asstring_t &), CScriptDictValue*), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("Dictionary", "const DictionaryValue &opIndex(const String &in) const", asMETHODPR(CScriptDictionary, operator[], (const asstring_t &) const, const CScriptDictValue*), asCALL_THISCALL); assert( r >= 0 );
	...
	// Same as delete
	r = engine->RegisterObjectMethod("Dictionary", "void erase(const String &in)", asMETHODPR(CScriptDictionary,Delete,(const asstring_t&), bool), asCALL_THISCALL); assert( r >= 0 );
	// Same as deleteAll
