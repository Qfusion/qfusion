...
void TVChannelsDataSource::GetRow( StringList &row, const String &table, int row_index, const StringList& cols ) {
	...
	for( StringList::const_iterator it = cols.begin(); it != cols.end(); ++it ) {
		if( *it == "id" ) {
			row.push_back( String( va( "%i", id ) ) );
		} else if( *it == "name" ) {
			row.push_back( chan_name.c_str() );
		} else if( *it == "players" ) {
			row.push_back( String( va( "%i", chan.numPlayers ) ) );
		} else if( *it == "spectators" ) {
			row.push_back( String( va( "%i", chan.numSpecs ) ) );
		} else if( *it == "map" ) {
			row.push_back( chan.mapname.c_str() );
		} else if( *it == "gametype" ) {
			row.push_back( chan.gametype.c_str() );
		} else if( *it == "matchname" ) {
			row.push_back( chan.matchname.c_str() );
		} else if( *it == "address" ) {
			row.push_back( chan.address.c_str() );
		} else if( *it == "complexname" ) {
			row.push_back( chan.matchname.empty() ? chan_name.c_str() : chan.matchname.c_str() );
		} else {
			row.push_back( "" );
		}
	}
