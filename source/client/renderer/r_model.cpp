...
static void R_LoadWorldRtLightsFromMap( model_t *model ) {
	...
	for(; ( token = COM_Parse( &data ) ) && token[0] == '{'; ) {
		...
		if( islight ) {
			...
			if( numLights == maxLights ) {
				...
				lights = ( rtlight_t * ) Mod_Realloc( lights, maxLights * sizeof( *lights ) );
			}
	...
	if( numLights ) {
		bmodel->rtLights = ( rtlight_t * ) Mod_Malloc( model, numLights * sizeof( rtlight_t ) );
...
static void R_LoadWorldRtSkyLights( model_t *model ) {
	...
	bmodel->rtSkyLights = ( rtlight_t * ) Mod_Malloc( model, numskies * sizeof( rtlight_t ) );
