...
float FragmentShadowValue(const uint si)
{
	vec3 shadowmaptc = vec3(v_ShadowProjVector[si].xyz / v_ShadowProjVector[si].w);

	// this keeps shadows from appearing on surfaces behind frustum's nearplane
	float d = step(v_ShadowProjVector[si].w, 0.0);

	//shadowmaptc = (shadowmaptc + vec3 (1.0)) * vec3 (0.5);
	shadowmaptc.xy = shadowmaptc.xy * u_ShadowmapTextureParams[si].xy; // .x - texture width
	shadowmaptc.z = clamp(shadowmaptc.z, 0.0, 1.0);
	shadowmaptc.xy = vec2(clamp(shadowmaptc.x, 0.0, u_ShadowmapTextureParams[si].x), clamp(shadowmaptc.y, 0.0, u_ShadowmapTextureParams[si].y));

	vec2 ShadowMap_TextureScale = u_ShadowmapTextureParams[si].zw;
	...
	# ifdef APPLY_PCF
	#  define texval(x, y) qfShadow2D(u_ShadowmapTexture[si], vec3(center + vec2(x, y)*ShadowMap_TextureScale, shadowmaptc.z))
	...
	# else
	f = qfShadow2D(u_ShadowmapTexture[si], vec3(shadowmaptc.xy*ShadowMap_TextureScale, shadowmaptc.z));
	# endif // APPLY_PCF
	...
	# ifdef APPLY_PCF
	# define texval(off) qfShadow2D(u_ShadowmapTexture[si], vec3(off,shadowmaptc.z))
...
#if MAX_SHADOWS >= 1
	s = FragmentShadowValue(0);
	finalcolor *= clamp(max(s, u_ShadowAlpha), 0.0, 1.0);
#endif
#if MAX_SHADOWS >= 2
	s = FragmentShadowValue(1);
	finalcolor *= clamp(max(s, u_ShadowAlpha), 0.0, 1.0);
#endif	
#if MAX_SHADOWS >= 3
	s = FragmentShadowValue(2);
	finalcolor *= clamp(max(s, u_ShadowAlpha), 0.0, 1.0);
#endif
#if MAX_SHADOWS >= 4
	s = FragmentShadowValue(3);
	finalcolor *= clamp(max(s, u_ShadowAlpha), 0.0, 1.0);
#endif
